...
void pro_input(const ProState *s, uint8_t timer, uint8_t out[50]) {
    ...
    out[2] = timer; out[3] = 0x90; memcpy(out + 4, s->buttons, 3);
    pack(out + 7, s->sticks[0], s->sticks[1]);
    pack(out + 10, s->sticks[2], s->sticks[3]); out[13] = 0xa0;
    if (s->imu) for (int i = 0; i < 3; i++) out[19 + 12*i] = 0x10;
}
static uint8_t spi(uint32_t a) {
    ...
    if (a >= 0x6080 && a < 0x6086) return motion[a - 0x6080];
    if (a >= 0x6086 && a < 0x6098) return params[a - 0x6086];
    if (a >= 0x6098 && a < 0x60aa) return params[a - 0x6098];
    if (a >= 0x6050 && a < 0x6053) return 0x82;
    if (a >= 0x6053 && a < 0x6056) return 0x0f;
    return 0xff;
}
bool pro_reply(ProState *s, const uint8_t *in, size_t len, uint8_t timer, uint8_t out[50]) {
    ...
    switch (cmd) {
    case 2:
        out[14]=0x82; out[16]=3; out[17]=0x8b; out[18]=3; out[19]=2;
    ...
    case 0x10: {
        ...
        for (unsigned i=0;i<in[16];i++) out[21+i]=spi(addr+i);
        break;
    }
    case 0x21: {
