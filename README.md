...
The first pairing requires **Controllers -> Change Grip/Order**. During that one-time
pairing the privileged backend also captures the BR/EDR Link Key generated by the
kernel and stores it privately under `/var/lib/pcble2gamepad/pairings/`. The stored
record contains the Switch address, local adapter identity, key type, PIN length and
the 128-bit Link Key encoded as Base64. Normal sessions restore that key into the
kernel before initiating HID control PSM 17 and interrupt PSM 19. This survives
application restarts, Stop session, host reboots and Switch reboots without opening
Change Grip/Order again.
